...
export const ComponentPreview = forwardRef<HTMLDivElement, ComponentPreviewProps>(
  ({ className, children, headline, description = "", code, ...args }, ref) => {
    ...
    return <section className="component-preview-container">
      ...
      <div ref={ref} className={cn("component-preview mt-6 min-h-[400px] justify-center border border-neutral-700 bg-white/10 rounded-2xl flex flex-row items-center relative w-full",
        className)} {...args}>
        <Button size="sm" variant="secondary" className="absolute top-2 right-2" onClick={() => setCodeView(!codeView)}>
          View Code
        </Button>
        {!codeView && children}
        {codeView && <CodeBlock>
